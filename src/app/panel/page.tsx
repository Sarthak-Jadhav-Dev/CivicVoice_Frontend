  ...
  return (
    <>
      <motion.div
        ...
        className="rounded-xl overflow-hidden border bg-white shadow-md relative flex flex-col justify-between"
      >
  ...
  return (
    <>
      <div className="flex h-screen w-full bg-gray-100">
        <motion.aside animate={{ width: sidebarOpen ? 240 : 64 }} className="h-full bg-white shadow-lg flex flex-col p-4 border-r overflow-hidden">
        ...
        <div className="flex-1 flex flex-col">
          <div className="flex flex-col gap-4 p-4 bg-white shadow-sm">
            <div className="flex items-center justify-end gap-4">
              <Select><SelectTrigger className="w-[140px]"><SelectValue placeholder="State" /></SelectTrigger><SelectContent><SelectItem value="ny">New York</SelectItem><SelectItem value="ca">California</SelectItem><SelectItem value="tx">Texas</SelectItem></SelectContent></Select>
              <Select><SelectTrigger className="w-[140px]"><SelectValue placeholder="City" /></SelectTrigger><SelectContent><SelectItem value="nyc">NYC</SelectItem><SelectItem value="la">Los Angeles</SelectItem><SelectItem value="houston">Houston</SelectItem></SelectContent></Select>
              <Select><SelectTrigger className="w-[140px]"><SelectValue placeholder="Area" /></SelectTrigger><SelectContent><SelectItem value="north">North</SelectItem><SelectItem value="south">South</SelectItem><SelectItem value="east">East</SelectItem><SelectItem value="west">West</SelectItem></SelectContent></Select>
              <Separator orientation="vertical" className="h-6" />
              <Select><SelectTrigger className="w-[180px]"><SelectValue placeholder="Issue Category" /></SelectTrigger><SelectContent><SelectItem value="infra">Infrastructure</SelectItem><SelectItem value="water">Water Supply</SelectItem><SelectItem value="electricity">Electricity</SelectItem><SelectItem value="waste">Waste Management</SelectItem></SelectContent></Select>
              {searchOpen && (<div className="flex items-center gap-2 w-[250px]"><Search className="h-4 w-4 text-gray-500" /><Input placeholder="Search reports..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full" /></div>)}
            </div>
          </div>

          <div className="flex-1 p-6 overflow-y-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {currentReports.map((report, idx) => (<ReportCard key={idx} {...report} index={idx} onResolve={handleResolve} />))}
            </div>
